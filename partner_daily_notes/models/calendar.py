...
class CalendarAppointment(models.Model):
    ...
    def generate_cancel_daily_note(self, cancel_reason, appointment):
        ...
        vals = {
            ...
            "note": _("Cancelled %s: %s. Reason: %s" % (self.type_id.name, self.start, cancel_reason.name)) if self.channel_name == "PDM" else _("Cancelled %s: %s, %s %s. Reason: %s" % (self.type_id.name, self.start, self.user_id.office_code, self.user_id.login, cancel_reason.name)),
    ...
    def generate_move_daily_note(self, occasions, reason):
        ...
        vals = {
            ...
            "partner_id": self.partner_id.id,
            "administrative_officer": self.user_id.id,
            "note": _("Meeting moved %s: %s. Reason: %s" % (self.type_id.name, self.start, reason.name)) if self.channel_name == "PDM" else _("Meeting moved %s: %s, %s %s. Reason: %s" % (self.type_id.name, self.start, self.user_id.office_code, self.user_id.login, reason.name)),
            "note_type": self.env.ref('partner_daily_notes.note_type_as_02').id,
            "office_id": self.partner_id.office_id.id,
            "appointment_id": self.id,
        }
    ...
    def create(self, values):
        ...
        if res.sudo().partner_id and res.state == 'confirmed':
            ...
            vals = {
                    ...
                    "partner_id": self.partner_id.id,
                    "administrative_officer": self.user_id.id,
                    "note":_("Booked %s: %s." % (self.type_id.name, self.start)) if self.channel_name == "PDM" else _("Booked %s: %s, %s %s." % (self.type_id.name, self.start, self.user_id.office_code, self.user_id.login)),
                    "note_type": self.env.ref('partner_daily_notes.note_type_as_02').id,
                    "office_id": self.partner_id.office_id.id,
                    "note_date": self.start,
                    "appointment_id": self.id,
...
class CalendarAppointmentSuggestion(models.Model):
    ...
    def select_suggestion(self):
        ...
        vals = {
                ...
                "note":_("Booked %s: %s." % (self.appointment_id.type_id.name, self.appointment_id.start)) if self.appointment_id.channel_name == "PDM" else _("Booked %s: %s, %s %s." % (self.appointment_id.type_id.name, self.appointment_id.start, self.appointment_id.user_id.office_code, self.appointment_id.user_id.login)),
                "note_type": self.env.ref('partner_daily_notes.note_type_as_02').id,
