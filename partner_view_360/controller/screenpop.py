...
class WebsiteScreenpop(http.Controller):
    ...
    def opencustomerview(self, **post):
        ...
        if request.env.user.login != post.get('signatur'):
            return request.render('partner_view_360.403',
                                  {'error': 'ERROR: Signature missmatch', 'signatur': post.get('signatur'),
                                   'partner': None, 'action': None, 'url': None, 'post': post, 'secret': secret})

        token = hashlib.sha1((secret + post.get('datatime', '0000-00-00-00') + post.get('personnummer', '').replace('-',
                                                                                                                    '') + post.get(
            'bankid', 'None')).encode('utf-8')).hexdigest()
        if not token == post.get('token'):
            return request.render('partner_view_360.403', {'error': 'ERROR: Token missmatch', 'our_token': token,
                                                           'ext_token': post.get('token'), 'partner': None,
                                                           'action': None, 'url': None, 'post': post, 'secret': secret,
        ...
        if not partner:
            return request.render('partner_view_360.403', {'error': 'ERROR: No partner found', 'our_token': token,
                                                           'ext_token': post.get('token'), 'partner': partner,
                                                           'action': None, 'post': post, 'secret': secret,
                                                           'signatur': post.get('signatur')})
            ...
            if res[0] != 250:  # OK
                return request.render('partner_view_360.403',
                                      {'error': 'ERROR: Escalate rights [%s] %s' % res, 'partner': partner,
                                       'signatur': post.get('signatur')})
            if post.get('debug'):
                return request.render('partner_view_360.403',
                                      {'message': 'Debug', 'our_token': token, 'ext_token': post.get('token'),
                                       'partner': partner, 'action': action, 'url': res_url, 'post': post,
                                       'secret': secret})
            return werkzeug.utils.redirect(res_url)
        else:
            return request.render('partner_view_360.403',
                                  {'error': 'ERROR: More than one partner found', 'our_token': token,
                                   'ext_token': post.get('token'), 'partner': partner, 'action': None, 'post': post,
                                   'secret': secret, 'signatur': post.get('signatur')})
    ...
    def opencustomerview_bankid(self, **post):
        ...
        if post.get('bankid_init'):
            ...
            bankid = CachingClient(request.env['ir.config_parameter'].sudo().get_param('partner_view_360.bankid_wsdl',
    ...
    def opencustomerviewtest(self, **post):
        ...
        secret = request.env['ir.config_parameter'].sudo().get_param('partner_view_360.secret', 'hemligt')
        # ~ token = hashlib.sha1(secret + fields.DateTime.now().tostring[:13].replace(' ','-') + post.get('personnummer') ).hexdigest()
        token = hashlib.sha1((secret + post.get('datatime', '0000-00-00-00') + post.get('personnummer',
                                                                                        '20010203-1234') + post.get(
            'bankid', '')).encode('utf-8')).hexdigest().upper()
        _logger.warn("\n\ntoken: %s" % token)
        ...
        res_url = '/web?id=%s&action=%s&model=res.partner&view_type=form#id=%s&active_id=40&model=res.partner&view_type=form' % (
    ...
    def opencustomerviewt2(self, **post):
        ...
        partner = request.env['res.partner'].sudo().search(
            [('company_registry', '=', post.get('personnummer', '20010203-1234'))])
        secret = request.env['ir.config_parameter'].sudo().get_param('partner_view_360.secret', 'hemligt')
        token = hashlib.sha1((secret + post.get('datatime', '0000-00-00-00') + post.get('personnummer',
                                                                                        '20010203-1234') + post.get(
            'bankid', '')).encode('utf-8')).hexdigest()
        token1 = hashlib.sha1((secret + post.get('datatime', '0000-00-00-00') + post.get('personnummer',
                                                                                         '20010203-1234') + post.get(
            'bankid', '')).encode('utf-8')).hexdigest().upper()

        return request.render('partner_view_360.403',
                              {'error': 'ERROR: No partner found', 'our_token': token, 'our_token1': token1,
                               'ext_token': post.get('token'), 'partner': partner, 'action': action, 'post': post,
                               'secret': secret})
