...
class ResPartner(models.Model):
    ...
    office_locations = fields.One2many('res.partner', related="office.locations") 
    my_locations = fields.One2many('res.partner') #should check if if it's in office_locations
    ...
    type = fields.Selection(selection_add=[('af office', 'AF Office'), ('foreign address','Foreign Address'), ('given address','Given address'), ('visitation address','Visitation Address'), ('location', 'Location')])

    # office code for office type partners only
    office_code = fields.Char(string="Office code")
    locations = fields.One2many('res.partner', string="Locations") # should check for type = "location"

    # Location code for location type partners only
