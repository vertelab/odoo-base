...
class BookingControllerExtended(BookingController):
    ...
    def booking_type_page_old(self, booking_type, **kwargs):
        ...
        """
        return request.redirect('/booking/%s?%s' % (request.env['ir.http']._unslug(booking_type)[1], keep_query('*')),
                                code=301)

    @route(['/booking/<int:booking_type_id>'],
           type='http', auth="public", website=True, sitemap=True)
    def booking_type_page(self, booking_type_id, state=False, staff_user_id=False, resource_selected_id=False,
                          **kwargs):
        """
        ...
        page_values = self._prepare_booking_type_page_values(booking_type, staff_user_id, resource_selected_id,
                                                             **kwargs)
    ...
    def _get_slots_values(self, booking_type, selected_filter_record, default_filter_record, possible_filter_records,
                          asked_capacity=1):
        """
    ...
    def _get_booking_type_page_view(self, booking_type, page_values, state=False, **kwargs):
        ...
        return request.render("base_booking_website.booking_info", render_params, headers={'Cache-Control': 'no-store'})

    def _prepare_booking_type_page_values(self, booking_type, staff_user_id=False, resource_selected_id=False,
                                          **kwargs):
        ...
        possible_combinations = (
                    resource_selected or resource_default or resources_possible)._get_filtered_possible_capacity_combinations(
            1, {})
        ...
        return {
            ...
            'hide_select_dropdown': len(
                users_possible if booking_type.schedule_based_on == 'users' else resources_possible) <= 1,
            'invite_token': kwargs.get('invite_token'),
    ...
    def booking_staff_user_avatar(self, booking_type_id, user_id=False, avatar_size=512):
        ...
        user = user if booking_type.avatars_display == 'show' and user in booking_type.staff_user_ids else request.env[
            'res.users']
    ...
    def booking_resource_avatar(self, booking_type_id, resource_id=False, avatar_size=512):
        ...
        resource = resource if booking_type.avatars_display == 'show' and resource in booking_type.resource_ids else \
    ...
    @staticmethod
    def _fetch_and_check_private_booking_types(booking_type_ids, staff_user_ids, resource_ids, invite_token,
                                               domain=False):
        """
    ...
    def booking_type_id_form(self, booking_type_id, date_time, duration, staff_user_id=None, resource_selected_id=None,
        ...
        if not self._check_booking_is_valid_slot(booking_type, staff_user_id, resource_selected_id,
                                                 available_resource_ids, date_time, duration, asked_capacity, **kwargs):
            raise NotFound()
        ...
        resource = request.env['booking.resource'].sudo().browse(int(resource_selected_id)) if resource_selected_id else \
        request.env['booking.resource']
    ...
    def _check_booking_is_valid_slot(self, booking_type, staff_user_id, resource_selected_id, available_resource_ids,
                                     start_dt, duration, asked_capacity, **kwargs):
        """
        ...
        try:
            ...
            staff_user_id = int(staff_user_id) if staff_user_id else False
            resource_selected_id = int(resource_selected_id) if resource_selected_id else False
            available_resource_ids = json.loads(
                unquote_plus(available_resource_ids)) if available_resource_ids else False
        ...
        return booking_type._check_booking_is_valid_slot(staff_user, resources, asked_capacity, session_tz,
                                                         start_dt_utc, duration)

    @http.route(['/booking/<int:booking_type_id>/submit'],
                type='http', auth="public", website=True, methods=["POST"])
    def booking_form_submit(self, booking_type_id, datetime_str, duration_str, name, phone, email, staff_user_id=None,
        ...
        """
        selected_resource = request.env['booking.resource'].sudo().browse(
            int(resource_selected_id)) if resource_selected_id else \
            request.env['booking.resource']
        ...
        if booking_type.schedule_based_on == 'resources':
            ...
            if any(resource not in booking_type.resource_ids for resource in resources):
                raise NotFound()
            resources_remaining_capacity = booking_type._get_resources_remaining_capacity(resources, date_start,
                                                                                          date_end,
            ...
            if staff_user and not staff_user.partner_id.calendar_verify_availability(date_start, date_end):
                return request.redirect(
                    '/booking/%s?%s' % (booking_type.id, keep_query('*', state='failed-staff-user')))
        ...
        if not new_customer:
            ...
            if not customer.phone:
                ...
                input_phone_fmt = phone_validation.phone_format(phone, input_country.code, input_country.phone_code,
    ...
    def get_upcoming_bookings(self, calendar_event_access_tokens=False):
        ...
        return {
            ...
            'valid_access_tokens': [appt['access_token'] for appt in
                                    upcoming_bookings] if request.env.user._is_public() else [],
        }
    ...
    def booking_slots(self, booking_type, employee_id=None, month=0, **kwargs):
        """
        Legacy route for JavaScript compatibility - returns paginated booking slots
        """
        booking_type_id = int(booking_type)
        domain = self._bookings_base_domain(
            filter_booking_type_ids=kwargs.get('filter_booking_type_ids'),
            search=kwargs.get('search'),
            invite_token=kwargs.get('invite_token')
        ...
        booking_type_obj = available_bookings.filtered(lambda appt: appt.id == booking_type_id)
        
        if not booking_type_obj:
            return False
            
        try:
            # Get paginated slots for the requested month
            booking_slots = booking_type_obj._get_paginated_booking_slots(
                request.session.get('timezone', booking_type_obj.booking_tz), 
                month=int(month)
            )
            ...
            return request.env['ir.qweb']._render('base_booking_website.booking_calendar', {
                'booking_type': booking_type_obj,
                'available_bookings': available_bookings,
                'timezone': request.session.get('timezone', booking_type_obj.booking_tz),
                'formated_days': formated_days,
                'slots': booking_slots,
                'month_first_available': 0,
                'month_kept_from_update': False,
            })
        except Exception:
            return False
    ...
    def booking_update_available_slots(self, booking_type_id, staff_user_id=None, resource_selected_id=None,
                                       asked_capacity=1, timezone=None, **kwargs):
        """
        ...
        asked_capacity = int(asked_capacity)
        slots = booking_type._get_booking_slots(request.session['timezone'], filter_users, filter_resources,
                                                asked_capacity=asked_capacity)
        month_first_available = next((month['id'] for month in slots if month['has_availabilities']), False)
        month_before_update = kwargs.get('month_before_update')
        month_kept_from_update = next((month['id'] for month in slots if month['month'] == month_before_update),
                                      False) if month_before_update else False
        formated_days = _formated_weekdays(get_lang(request.env).code)
