...
class BookingType(models.Model):
    ...
    def _default_booked_mail_template_id(self):
        return self.env['ir.model.data']._xmlid_to_res_id('base_booking.attendee_invitation_mail_template')

    def _default_canceled_mail_template_id(self):
        return self.env['ir.model.data']._xmlid_to_res_id('base_booking.booking_canceled_mail_template')
    ...
    booked_mail_template_id = fields.Many2one(
        ...
        help="If set an email will be sent to the customer when the booking is confirmed.")
    canceled_mail_template_id = fields.Many2one(
        ...
        help="If set an email will be sent to the customer when the booking is cancelled.")
    ...
    category = fields.Selection([
        ...
        help="""Used to define this booking type's category.\n
        Can be one of:\n
            ...
            - Custom: the user will create and share to another user a custom booking type with hand-picked time slots\n
            - Anytime: the user will create and share to another user an booking type covering all their time slots""")
    ...
    country_ids = fields.Many2many(
        'res.country', 'booking_type_country_rel', string='Allowed Countries',
    ...
    message_confirmation = fields.Html('Confirmation Message', translate=True,
        help="Extra information provided once the booking is booked.")
    message_intro = fields.Html('Introduction Message', translate=True,
        sanitize_attributes=False,
        help="Small description of the booking type.")
    ...
    slot_ids = fields.One2many('booking.slot', 'booking_type_id', 'Availabilities', copy=True)

    # Staff Users Management
    staff_user_ids = fields.Many2many(
        'res.users',
        'booking_type_res_users_rel',
        domain="[('share', '=', False)]",
    ...
    resource_ids = fields.Many2many('booking.resource', string="Resources",
        relation="booking_type_booking_resource_relation",
    ...
    # Technical field for backward compatibility with previous default published booking type
    ...
    message_partner_ids = fields.Many2many(string='CC to',
                                           help="Contacts that need to be notified whenever a new booking is booked or cancelled, \
    ...
    def _compute_booking_counts(self):
        ...
        status_data = self.env['calendar.event']._read_group(
            [('booking_type_id', 'in', self.ids)],
            ['booking_type_id', 'booking_status'], ['__count']
        )
        for booking_type, status, count in status_data:
            if booking_type.id not in mapped_status_data:
                mapped_status_data[booking_type.id] = {}
            mapped_status_data[booking_type.id][status] = count

        mapped_upcoming_data = {
            booking_type.id: count for booking_type, count in self.env['calendar.event']._read_group(
                [('booking_type_id', 'in', self.ids), ('start', '>', datetime.now())],
                ['booking_type_id'], ['__count'])
        }
        for booking_type in self:
            booking_status_data = mapped_status_data.get(booking_type.id, {'booked': 0})
            booking_type.booking_count = sum(booking_status_data.values())
            booking_type.booking_count_request = booking_status_data.get('request', 0)
            booking_type.booking_count_upcoming = mapped_upcoming_data.get(booking_type.id, 0)

    @api.depends('booking_duration')
    def _compute_booking_duration_formatted(self):
        """
        When booking duration exceeds a day or is not a multiple of hours e.g., 2 hours 30 minutes,
        ...
        """
        for record in self:
            record.booking_duration_formatted = self.env['ir.qweb.field.duration'].value_to_html(
                record.booking_duration * 3600,
                {} if record.booking_duration % 1 == 0 and record.booking_duration < 24 else {'format': 'short'}
            )

    @api.depends('booking_invite_ids')
    def _compute_booking_invite_count(self):
        booking_data = self.env['booking.invite']._read_group(
            [('booking_type_ids', 'in', self.ids)],
            ['booking_type_ids'],
            ['__count'],
        )
        mapped_data = {booking_type.id: count for booking_type, count in booking_data}
        for booking_type in self:
            booking_type.booking_invite_count = mapped_data.get(booking_type.id, 0)

    @api.depends('category')
    def _compute_avatars_display(self):
        """ By default, enable avatars for custom booking types and hide them for recurring and punctual category ones."""
        for record in self:
    ...
    def _compute_category(self):
        for booking_type in self:
            booking_type.category = 'punctual' if booking_type.start_datetime or booking_type.end_datetime else 'recurring'
            if not booking_type.slot_ids:
                booking_type.slot_ids = booking_type._get_default_slots(booking_type.category)

    def _inverse_category(self):
        """ Generate the default slots for the anytime booking types.
        If the category is 'custom', no need to generate default slots. """
        anytime_booking_types = self.filtered_domain([('category', '=', 'anytime')])
        anytime_booking_types.slot_ids = False # Reset slots if existing
        for booking_type in anytime_booking_types:
            booking_type.slot_ids = booking_type._get_default_slots('anytime')

    @api.depends('category')
    def _compute_category_time_display(self):
        for booking_type in self:
            booking_type.category_time_display = 'punctual_fields' if booking_type.category == 'punctual' else 'recurring_fields'
    ...
    def _compute_resource_ids(self):
        for booking_type in self.filtered(lambda appt: appt.schedule_based_on == 'users'):
            booking_type.resource_ids = False
    ...
    def _compute_connectors_displayed(self):
        ...
        for booking_type in self:
            booking_type.connectors_displayed = (
                connectors_enabled and
                booking_type.schedule_based_on == 'users' and
                booking_type.env.user in self.staff_user_ids
            )

    @api.depends('schedule_based_on')
    def _compute_staff_user_ids(self):
        for booking_type in self.filtered(lambda appt: appt.schedule_based_on == 'resources'):
            booking_type.staff_user_ids = False

    @api.depends('resource_ids', 'resource_ids.capacity')
    def _compute_resource_info(self):
        resource_data = self.env['booking.resource']._read_group(
            [('booking_type_ids', 'in', self.ids)],
            ['booking_type_ids'],
        ...
        mapped_data = {
            ...
            } for booking_type, count, total_capacity in resource_data}

        for booking_type in self:
            if isinstance(booking_type.id, models.NewId):
                booking_type.resource_count = len(booking_type.resource_ids)
                booking_type.resource_total_capacity = sum(resource.capacity for resource in booking_type.resource_ids)
            else:
                booking_type_data = mapped_data.get(booking_type.id, {})
                booking_type.resource_count = booking_type_data.get('count', 0)
                booking_type.resource_total_capacity = booking_type_data.get('total_capacity', 0)

    @api.depends('schedule_based_on')
    def _compute_resource_manage_capacity(self):
        for booking_type in self:
            if booking_type.schedule_based_on == 'users':
                booking_type.resource_manage_capacity = False
    ...
    def _check_booking_category_time_boundaries(self):
        for booking_type in self:
            if booking_type.category == 'punctual' and not (booking_type.start_datetime and booking_type.end_datetime):
                raise ValidationError(_('A punctual booking type should be limited between a start and end datetime.'))
            elif booking_type.category != 'punctual' and (booking_type.start_datetime or booking_type.end_datetime):
                raise ValidationError(_("A %s booking type shouldn't be limited by datetimes.", booking_type.category))
            if booking_type.start_datetime and booking_type.end_datetime and booking_type.start_datetime > booking_type.end_datetime:
                raise ValidationError(_("Start date should precede the end date."))

    @api.constrains('booking_duration')
    ...
    def _check_staff_user_configuration(self):
        anytime_bookings = self.search([('category', '=', 'anytime')])
        for booking_type in self.filtered(lambda appt: appt.schedule_based_on == "users" and appt.category == "anytime"):
            duplicate = anytime_bookings.filtered(lambda apt_type: bool(apt_type.staff_user_ids & booking_type.staff_user_ids))
            if booking_type.ids != duplicate.ids:
                raise ValidationError(_("Only one anytime booking type is allowed for a specific user."))

    @api.model_create_multi
    def create(self, vals_list):
        """ We don't want the current user to be follower of all created types """
        return super(BookingType, self.with_context(mail_create_nosubscribe=True)).create(vals_list)

    def copy_data(self, default=None):
        vals_list = super().copy_data(default=default)
        return [dict(
            vals,
            name=self.env._("%s (copy)", booking_type.name),
            category=booking_type.category,
        ) for booking_type, vals in zip(self, vals_list)]

    def action_booking_resources(self):
        self.ensure_one()
        action = self.env["ir.actions.act_window"]._for_xml_id("base_booking.booking_resource_action")
        action["domain"] = [('booking_type_ids', 'in', self.ids)]
        action["context"] = {
            'default_booking_type_ids': self.ids,
        }
        return action

    def action_booking_shared_links(self):
        action = self.env["ir.actions.act_window"]._for_xml_id("base_booking.booking_invite_action")
        action["domain"] = [('booking_type_ids', 'in', self.ids)]
        if len(self) == 1:
    ...
    def action_calendar_event_view_request(self):
        action = self.action_calendar_meetings(calendar_event_domain=[('booking_status', '=', 'request')])
        action['context'].update({
            'search_default_filter_booking_status_request': True,
    ...
    def action_calendar_events_reporting(self):
        self.ensure_one()
        action = self.env["ir.actions.act_window"]._for_xml_id("base_booking.calendar_event_action_booking_reporting")
        action["domain"] = [('booking_type_id', '!=', False)]
        action["context"] = {
    ...
    def action_calendar_meetings(self, calendar_event_domain=False):
        ...
        bookings = self.meeting_ids.filtered_domain(domain)
        nbr_bookings_week_later = bookings.filtered_domain([
        ...
        action['context'].update({
            ...
            'default_booking_type_id': self.id,
            'default_duration': self.booking_duration,
            'default_partner_ids': [],
            'search_default_booking_type_id': self.id,
    ...
    def action_calendar_meetings_users_all(self):
        action = self.env["ir.actions.actions"]._for_xml_id("base_booking.calendar_event_action_view_bookings_users")
        action = BookingType.insert_reorder_action_views(action, ['gantt'])
        action['context'] = ast.literal_eval(action['context'])
        action['context'].update({
    ...
    def action_customer_preview(self):
        ...
        return {
            ...
            'url': url_join(self.get_base_url(), '/booking/%s' % self.id),
            'target': 'self',
        }
    ...
    def _get_default_range_slots(self, category):
        '''
            If the booking type is of category recurring or punctual, we set the arbitrary 'standard'
            booking slots range (from monday to friday, 9AM-12PM and 2PM-5PM).
            If the booking type is of category anytime, we set the slots range
            as any time between 2 arbitrary hours (monday to sunday, 7AM-7PM).
            The slot range for the anytime category will be updated in booking_hr
            to match the user work hours.
        '''
        if category not in ['punctual', 'recurring', 'anytime']:
            raise ValueError(_("Default slots cannot be applied to the %s booking type category.", category))
    ...
    def _get_default_booking_status(self, start_dt, stop_dt, capacity_reserved):
        """ Get the status of the booking based on users/resources and the manual confirmation option.
        :param datetime start_dt: start datetime of booking (in naive UTC)
        :param datetime stop_dt: stop datetime of booking (in naive UTC)
        :param int capacity_reserved: capacity reserved by the customer for the booking
        """
        self.ensure_one()
        default_state = 'booked'
        if self.booking_manual_confirmation and self.schedule_based_on == 'resources':
            bookings_data = self.env['booking.line'].sudo()._read_group([
        ...
        elif self.booking_manual_confirmation:
            default_state = 'request'
    ...
    def _slots_generate(self, first_day, last_day, timezone, reference_date=None):
        ...
        :param datetime first_day: beginning of booking check boundary. Timezoned to UTC;
        :param datetime last_day: end of booking check boundary. Timezoned to UTC;
        ...
        :param datetime reference_date: starting datetime to fetch slots (in naive UTC). If not
          ...
          defined on booking type is added to the beginning of slots if the
        ...
        def append_slot(day, slot):
            ...
            :param record slot: a <booking.slot> record
            """
            ...
            while local_start < ref_start:
                local_start += relativedelta(hours=self.booking_duration)

            local_end = local_start + relativedelta(hours=self.booking_duration)
            ...
            # Adapt local_slot_end to not append slot if local_slot_end is in the future of the booking end_datetime
            ...
            n_slot = int(end_start_delta / self.booking_duration)
            for _index in range(n_slot):
                ...
                local_end += relativedelta(hours=self.booking_duration)

        # We use only the recurring slot if it's not a custom booking type.
        if self.category != 'custom':

            # Don't generate slots if the booking boundaries are completely in the past
            if last_day < reference_date.astimezone(pytz.UTC):
                return slots

            # Regular recurring slots (not a custom booking), generate necessary slots using configuration rules
            ...
            # Custom booking type, we use "unique" slots here that have a defined start/end datetime
    ...
    def _get_booking_slots(self, timezone, filter_users=None, filter_resources=None, asked_capacity=1, reference_date=None):
        ...
        :param <res.users> filter_users: filter available slots for those users (can be a singleton
          for fixed booking types or can contain several users, e.g. with random assignment and
          filters) If not set, use all users assigned to this booking type.
        :param <booking.resource> filter_resources: filter available slots for those resources
          (can be a singleton for fixed booking types or can contain several resources,
        ...
        :param datetime reference_date: starting datetime to fetch slots. If not
          ...
          defined on booking type is added to the beginning of slots;
        ...
        try:
            ...
            requested_tz = self.booking_tz

        booking_duration_days = self.max_schedule_days
        unique_slots = self.slot_ids.filtered(lambda slot: slot.slot_type == 'unique')

        if self.category == 'custom' and unique_slots:
            # Custom booking type, the first day should depend on the first slot datetime
            ...
            booking_duration_days = (unique_slots[-1].end_datetime.date() - reference_date.date()).days
            last_day = requested_tz.fromutc(reference_date + relativedelta(days=booking_duration_days))
        elif self.category == 'punctual':
            # Punctual booking type, the first day is the start_datetime if it is in the future, else the first day is now
        ...
        last_day_end_of_day = datetime.combine(
            last_day.astimezone(pytz.timezone(self.booking_tz)),
            time.max
        )
        if self.schedule_based_on == 'users':
    ...
    def _check_booking_is_valid_slot(self, staff_user, resources, asked_capacity, timezone, start_dt, duration):
        """
        Given slot parameters check if it is still valid, based on employee
        availability, slot boundaries, ...
        :param (optional record) staff_user: the user for whom the booking was booked for
        :param (optional record) resources: the resources for which the booking was booked for
        :param integer asked_capacity: the capacity asked by the customer
        :param str timezone: visitor's timezone
        :param datetime start_dt: start datetime of the booking (UTC)
        :param float duration: the duration of the booking in hours
        :return: True if at least one slot is available, False if no slots were found
        """
        # the user can be a public/portal user that doesn't have read access to the booking_type.
        self_sudo = self.sudo()
        ...
        for slot in slots:
            ...
            if resources and any(resource not in slot.get("available_resource_ids", []) for resource in resources):
                continue
            if slot['slot'].slot_type == 'recurring' and float_compare(self_sudo.booking_duration, duration, 2) != 0:
                continue
    ...
    @api.model
    def _get_clean_booking_context(self):
        whitelist_default_fields = list(map(
            lambda field: f'default_{field}',
            self._get_calendar_view_booking_type_default_context_fields_whitelist()))
        return {
    ...
    @api.model
    def _get_calendar_view_booking_type_default_context_fields_whitelist(self):
        """ White list of fields that can be defaulted in the context of the
        calendar routes creating booking types and invitations.
        This is mainly used in /booking/booking_type/create_custom and
        /booking/booking_type/search_create_anytime.
    ...
    def _prepare_calendar_event_values(
        ...
        """ Returns all values needed to create the calendar event from the values outputed
            ...
            matching booking_form_submit controller's ones.
            ...
        ...
        booking_status = self._get_default_booking_status(start, stop, asked_capacity)
        ...
        return {
            ...
            'name': _('%(attendee_name)s - %(booking_name)s Booking',
                        attendee_name=name, booking_name=self.name),
    ...
    def _slots_fill_users_availability(self, slots, start_dt, end_dt, filter_users=None):
        ...
        :param datetime start_dt: beginning of booking check boundary. Timezoned to UTC;
        :param datetime end_dt: end of booking check boundary. Timezoned to UTC;
        :param <res.users> filter_users: filter available slots for those users (can be a singleton
          for fixed booking types or can contain several users e.g. with random assignment and
          filters) If not set, use all users assigned to this booking type.
    ...
    def _slot_availability_is_user_available(self, slot, staff_user, availability_values):
        ...
        :return: boolean: is user available for an booking for given slot
        """
    ...
    def _slot_availability_prepare_users_values(self, staff_users, start_dt, end_dt):
        ...
        :param <res.users> staff_users: prepare values to check availability
          of those users against given booking boundaries. At this point
    ...
    def _slots_fill_resources_availability(self, slots, start_dt_utc, end_dt_utc, filter_resources=None, asked_capacity=1):
        ...
        :param list slots: slots (list of slot dict), as generated by ``_slots_generate``;
        :param datetime start_dt_utc: beginning of booking check boundary. Timezoned to UTC;
        :param datetime end_dt_utc: end of booking check boundary. Timezoned to UTC;
        :param <booking.resource> filter_resources: filter available slots for those resources (can be a singleton
          for fixed booking types or can contain several resources)
          If not set, use all resources assigned to this booking type.
        :param integer asked_capacity: asked capacity for the booking
        ...
        available_resources = self.env['booking.resource'].concat(*available_resources)
    ...
    def _slot_availability_is_resource_available(self, slot, resource, availability_values):
        ...
        :param dict slot: a slot as generated by ``_slots_generate``;
        :param <booking.resource> resource: resource to check against slot boundaries.
          At this point timezone should be correctly set in context;
        :param dict availability_values: dict of data used for availability check.
          See ``_slot_availability_prepare_resources_values()`` for more details;

        :return: boolean: is resource available for an booking for given slot
        """
    ...
    def _get_resources_remaining_capacity(self, resources, slot_start_utc, slot_stop_utc, resource_to_bookings=None, with_linked_resources=True, filter_resources=None):
        """ Compute the remaining capacities for resources in a particular time slot.
            :param <booking.resource> resources : record containing one or a multiple of resources
            ...
            :param list resource_to_bookings: list of resource linked to their booking lines from the prepared value.
                If no value is passed, then we search manually the booking lines (used for the booking validation step)
            :param bool with_linked_resources: If true we take into account the linked resources for the computation.
                ...
                booking or to compute remaining capacity for a particular resource)
            :param <booking.resource> filter_resources: filter the resources impacted with this value
        ...
        booking_lines = self.env['booking.line'].sudo()
        if resource_to_bookings is None:
            booking_lines = self.env['booking.line'].sudo().search([
                ('booking_resource_id', 'in', all_resources.ids),
        ...
        resources_booking_lines = booking_lines.grouped('booking_resource_id')
    ...
    def _slot_availability_select_best_resources(self, capacity_info, asked_capacity):
        """ Check and select the best resources for the capacity needed
            ...
            :params asked_capacity <integer>: asked capacity for the booking
            :returns: we return recordset of best resources selected
        """
        self.ensure_one()
        available_resources = self.env['booking.resource'].concat(*capacity_info.keys()).sorted('sequence')
        if not available_resources:
            return self.env['booking.resource']
    ...
    def _slot_availability_prepare_resources_values(self, resources, start_dt_utc, end_dt_utc):
        ...
        :param <booking.resource> resources: prepare values to check availability
          of those resources against given booking boundaries. At this point
    ...
    def _slot_availability_prepare_resources_bookings_values(self, resources, start_dt_utc, end_dt_utc):
        ...
        of booking check. Also, resources can be shared between multiple booking
        ...
        :return: dict containing main values for computation, formatted like
          {
            'resource_to_bookings': bookings, formatted as a dict
              {
                'booking_resource_id': recordset of booking line,
                ...
              },
          }
        """
        ...
        if resources:
            booking_lines = self.env['booking.line'].sudo().search([
                ('booking_resource_id', 'in', resources.ids),
                ('event_stop', '>', datetime.combine(start_dt_utc, time.min)),
                ('event_start', '<', datetime.combine(end_dt_utc, time.max))])
            resource_to_bookings = booking_lines.grouped('booking_resource_id')

        return {
            'resource_to_bookings': resource_to_bookings,
        }

    def _slot_availability_prepare_resources_leave_values(self, booking_resources, start_dt_utc, end_dt_utc):
        """Retrieve a list of unavailabilities for each resource.

        :param <booking.resource> booking_resources: resources to get unavalabilities for;
        :param datetime start_dt_utc: beginning of booking check boundary. Timezoned to UTC;
        :param datetime end_dt_utc: end of booking check boundary. Timezoned to UTC;
        ...
        """
        unavailabilities = booking_resources.sudo().resource_id._get_unavailable_intervals(start_dt_utc, end_dt_utc)
        return {'resource_unavailabilities': {
            resource: unavailabilities.get(resource.sudo().resource_id.id, []) for resource in booking_resources
        }}
