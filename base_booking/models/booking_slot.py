...
class BookingSlot(models.Model):
    ...
    schedule_based_on = fields.Selection(related="booking_type_id.schedule_based_on")
    slot_type = fields.Selection([('recurring', 'Recurring'), ('unique', 'One Shot')],
        ...
        booking type that are used recurringly in type like medical booking.
        The one shot type is only used when an user create a custom booking type for a client by
    ...
    restrict_to_resource_ids = fields.Many2many("booking.resource", string="Restrict to Resources",
    ...
    def _compute_slot_type(self):
        for slot in self:
            slot.slot_type = 'unique' if slot.booking_type_id.category == 'custom' else 'recurring'

    @api.depends('start_hour')
    def _compute_end_hour(self):
        """ Try to adapt end_hour if the interval end_hour < start_hour """
        for record in self:
            duration = record.booking_type_id.booking_duration
    ...
    def _check_delta_hours(self):
        if any(self.filtered(lambda slot: slot.start_hour + slot.booking_type_id.booking_duration > slot._convert_end_hour_24_format() and slot.slot_type != 'unique')):
            raise ValidationError(_(
                "At least one slot duration is shorter than the meeting duration (%s hours)",
                format_duration(self.booking_type_id.booking_duration)
            ))
    ...
    @api.depends('booking_type_id.resource_ids')
    def _compute_restrict_to_resource_ids(self):
        for slot in self:
            slot.restrict_to_resource_ids &= slot.booking_type_id.resource_ids

    @api.depends('booking_type_id.staff_user_ids')
    def _compute_restrict_to_user_ids(self):
        for slot in self:
            slot.restrict_to_user_ids &= slot.booking_type_id.staff_user_ids
