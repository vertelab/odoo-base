...
class Attendee(models.Model):
    ...
    def _compute_mail_tz(self):
        toupdate = self.filtered(lambda r: r.event_id.booking_type_id.appointment_tz)
        for attendee in toupdate:
            attendee.mail_tz = attendee.event_id.booking_type_id.appointment_tz
    ...
    def _send_invitation_emails(self):
        ...
        appointment_attendees = self.filtered(lambda attendee: attendee.event_id.booking_type_id)
        super(Attendee, self - appointment_attendees)._send_invitation_emails()

        attendees_per_appointment_type = tools.groupby(
            appointment_attendees,
            lambda attendee: attendee.event_id.booking_type_id)
    ...
    def _should_notify_attendee(self):
        ...
        return self.event_id.booking_type_id or super()._should_notify_attendee()
