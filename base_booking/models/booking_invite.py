...
class BookingShare(models.Model):
    ...
    # Put active_test to False because we always want to be able to check all booking types from an invitation.
    # In case the booking type is archived, we still want old links to work and display with a message telling
    # that it's no longer available.
    booking_type_ids = fields.Many2many('booking.type', string='Booking Types', context={"active_test": False})
    booking_type_info_msg = fields.Html('No User Assigned Message', compute='_compute_booking_type_info_msg')
    booking_type_count = fields.Integer('Selected Bookings Count', compute='_compute_booking_type_count', store=True)
    schedule_based_on = fields.Char('Schedule Based On', compute="_compute_schedule_based_on")
    suggested_resource_ids = fields.Many2many('booking.resource', related="booking_type_ids.resource_ids", string="Possible resources")
    ...
    suggested_staff_user_ids = fields.Many2many(
        'res.users', related='booking_type_ids.staff_user_ids', string='Possible users',
        help="Get the users linked to the booking type selected to apply a domain on the users that can be selected")
    ...
    resource_ids = fields.Many2many('booking.resource', string='Resources', domain="[('id', 'in', suggested_resource_ids)]",
    ...
    calendar_event_ids = fields.One2many('calendar.event', 'booking_invite_id', string="Booked Bookings", readonly=True)
    ...
    def _compute_disable_save_button(self):
        for invite in self:
            conditions = [
                ...
                invite.booking_type_count == 1 and invite.resources_choice == 'current_user' and self.env.user.id not in invite.suggested_staff_user_ids.ids,
                not invite.suggested_staff_user_ids and invite.booking_type_count == 1 and invite.suggested_resource_count < 1,
            ]
    ...
    @api.depends('booking_type_ids')
    def _compute_schedule_based_on(self):
        """ Get the schedule_based_on value when selecting one booking type.
        This allows to personalize the warning or info message based on this value. """
        for invite in self:
            invite.schedule_based_on = invite.booking_type_ids.schedule_based_on if len(invite.booking_type_ids) == 1 else False

    @api.depends('booking_type_ids', 'booking_type_count')
    def _compute_booking_type_info_msg(self):
        '''
            When there is more than one booking type selected to be shared and at least one doesn't have any staff user or resource assigned,
            display an alert info to tell the current user that, without staff users or resources, an booking type won't be published.
        '''
        for invite in self:
            appt_without_staff_user = invite.booking_type_ids.filtered_domain([('schedule_based_on', '=', 'users'), ('staff_user_ids', '=', False)])
            appt_without_resource = invite.booking_type_ids.filtered_domain([('schedule_based_on', '=', 'resources'), ('resource_ids', '=', False)])
            booking_type_info_msg = Markup()
            if appt_without_staff_user and invite.booking_type_count > 1:
                booking_type_info_msg += _(
                    'The following booking type(s) have no staff assigned: %s.',
                    ', '.join(appt_without_staff_user.mapped('name'))
                ) + Markup('<br/>')
            if appt_without_resource and invite.booking_type_count > 1:
                booking_type_info_msg += _(
                    'The following booking type(s) have no resource assigned: %s.',
                    ', '.join(appt_without_resource.mapped('name'))
                )
            invite.booking_type_info_msg = booking_type_info_msg if booking_type_info_msg else False

    @api.depends('booking_type_ids')
    def _compute_booking_type_count(self):
        booking_data = self.env['booking.type']._read_group(
            [('booking_invite_ids', 'in', self.ids)],
            ['booking_invite_ids'],
            ['__count'],
        )
        mapped_data = {booking_invite.id: count for booking_invite, count in booking_data}
        for invite in self:
            if isinstance(invite.id, models.NewId):
                invite.booking_type_count = len(invite.booking_type_ids)
            else:
                invite.booking_type_count = mapped_data.get(invite.id, 0)
    ...
    def _compute_calendar_event_count(self):
        booking_invite_data = self.env['calendar.event']._read_group(
            [('booking_invite_id', 'in', self.ids)],
            ['booking_invite_id'],
            ['__count'],
        )
        mapped_data = {invite.id: count for invite, count in booking_invite_data}
        for invite in self:
    ...
    def _compute_short_code_warning(self):
        for invite in self:
            ...
            invite.short_code_unique_warning = bool(self.env['booking.invite'].search_count([
                ('id', '!=', invite._origin.id), ('short_code', '=', invite.short_code)]))

    @api.depends('booking_type_ids')
    def _compute_resources_choice(self):
        for invite in self:
            if len(invite.booking_type_ids) != 1:
                invite.resources_choice = False
            elif invite.booking_type_ids.schedule_based_on == 'users' and self.env.user in invite.booking_type_ids._origin.staff_user_ids:
    ...
    def _compute_resource_ids(self):
        for invite in self:
            if len(invite.booking_type_ids) > 1 or invite.booking_type_ids.schedule_based_on != 'resources':
                invite.resource_ids = False

    @api.depends('booking_type_ids', 'resources_choice')
    def _compute_staff_user_ids(self):
        for invite in self:
            if invite.resources_choice == "current_user" and \
                    self.env.user.id in invite.booking_type_ids.staff_user_ids.ids:
    ...
    def _compute_book_url(self):
        """
        Compute a short link linked to an booking invitation.
        """
        for invite in self:
            invite.book_url = url_join(invite.base_book_url, invite.short_code) if invite.short_code else False

    @api.depends('booking_type_ids', 'staff_user_ids', 'resource_ids')
    def _compute_redirect_url(self):
        """
        Compute a link that will be share for the user depending on the booking types and users
        selected. We allow to preselect a group of them if there is only one booking type selected.
        Indeed, it would be too complex to manage ones with multiple booking types.
        Three possible params can be generated with the link:
            ...
            - filter_booking_type_ids: which display a selection of booking types to user from which
            they can choose
        """
        for invite in self:
            if len(invite.booking_type_ids) == 1:
                base_redirect_url = url_join("/booking/", str(invite.booking_type_ids.id))
            else:
                base_redirect_url = '/booking'
    ...
    def _get_invitation_url_parameters(self):
        ...
        return {'filter_booking_type_ids', 'filter_resource_ids', 'filter_staff_user_ids', 'invite_token'}
    ...
    def _get_redirect_url_parameters(self):
        ...
        if self.booking_type_ids:
            url_param.update({
                'filter_booking_type_ids': str(self.booking_type_ids.ids),
            })
    ...
    def _check_bookings_params(self, booking_types, users, resources):
        """
        Check if the param receive through the URL match with the booking invite info
        :param recordset booking_types: the booking types representing the filter_booking_type_ids
        ...
        """
        self.ensure_one()
        if (self.booking_type_ids and self.booking_type_ids != booking_types) or self.staff_user_ids != users or self.resource_ids != resources:
            return False
        return True
    ...
    def _get_unique_short_code(self, short_code=None):
        ...
        nb_short_code = self.env['booking.invite'].search_count([('id', '!=', self._origin.id), ('short_code', '=', short_code)])
        if nb_short_code:
            short_code = "%s_%s" % (short_code, nb_short_code)
        return short_code

    @api.autovacuum
    def _gc_booking_invite(self):
        limit_dt = fields.Datetime.subtract(fields.Datetime.now(), months=6)

        invites = self.env['booking.invite'].search([('create_date', '<=', limit_dt)])
