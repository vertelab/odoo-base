...
class BookingType(models.Model):
    '''
    This file inherits the booking.type model and centralize all methods linked
    to booking type templates in a separate file. These templates are used to
    ease the onboarding and load specific configurations, hinted by their description.
    They appear instead of the helper on booking.type list and kanban views when
    no booking exists.
    '''
    _inherit = 'booking.type'

    @api.model
    def action_setup_booking_type_template(self, template_key):
        action = self.env['ir.actions.act_window']._for_xml_id('booking.booking_type_action')
        template_values = self._get_booking_type_template_values(template_key)
        action['res_id'] = self.env['booking.type'].create(template_values).id
        action['views'] = [[self.env.ref('booking.booking_type_view_form').id, 'form']]
        return action
