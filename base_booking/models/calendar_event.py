...
from odoo.addons.base_booking.utils import invert_intervals
...
class CalendarEvent(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if res.get('booking_type_id') and res.get('duration') and res.get('start') and 'stop' in fields_list:
        ...
        # get a relevant booking type for ease of use when coming from a view that groups by resource
        if not res.get('booking_type_id') and 'booking_type_id' in fields_list:
            booking_types = False
            if resource_ids:
                booking_types = self.env['booking.resource'].browse(resource_ids).booking_type_ids
            elif user_id:
                booking_types = self.env['booking.type'].search([('staff_user_ids', 'in', user_id)])
            if booking_types:
                res['booking_type_id'] = booking_types[0].id
        if self.env.context.get('booking_default_assign_user_attendees'):
            ...
            # Mostly used when you click on a specific slot in the booking kanban
    ...
    # selected. This allows to have the booking_resource_ids field linked to the booking_booking_line model and
    ...
    resource_ids = fields.Many2many('booking.resource', string="Resources",
                                    ...
                                    group_expand="_read_group_booking_resource_ids")
    booking_line_ids = fields.One2many('booking.line', 'calendar_event_id', string="Booking Lines")
    ...
    booking_booker_id = fields.Many2one('res.partner', string="Person who is booking the booking", index='btree_not_null')
    on_leave_partner_ids = fields.Many2many('res.partner', string='Unavailable Partners', compute='_compute_on_leave_partner_ids')
    on_leave_resource_ids = fields.Many2many('booking.resource', string='Resources intersecting with leave time', compute="_compute_on_leave_resource_ids")

    @api.constrains('booking_resource_ids', 'booking_type_id')
    def _check_resource_and_booking_type(self):
        for event in self:
            if event.booking_resource_ids and not event.booking_type_id:
                raise ValidationError(_("The event %s cannot book resources without an booking type.", event.name))

    @api.constrains('booking_type_id', 'booking_status')
    def _check_status_and_booking_type(self):
        for event in self:
            if event.booking_status and not event.booking_type_id:
                raise ValidationError(_("The event %s cannot have an booking status without being linked to an booking type.", event.name))

    @api.depends('booking_type_id')
    def _compute_alarm_ids(self):
        for event in self.filtered('booking_type_id'):
            if not event.alarm_ids:
                event.alarm_ids = event.booking_type_id.reminder_ids

    @api.depends('booking_type_id')
    def _compute_booking_status(self):
        for event in self:
            if not event.booking_type_id:
                event.booking_status = False
            elif not event.booking_status:
                event.booking_status = 'booked'

    @api.depends('booking_line_ids', 'booking_line_ids.booking_resource_id')
    def _compute_resource_ids(self):
        for event in self:
            event.resource_ids = event.booking_line_ids.booking_resource_id

    @api.depends('start', 'stop', 'partner_ids')
    def _compute_on_leave_partner_ids(self):
        self.on_leave_partner_ids = False
        user_events = self.filtered(lambda event: event.booking_type_id.schedule_based_on == 'users')
    ...
    def _compute_resource_total_capacity(self):
        booking_data = self.env['booking.line']._read_group(
    ...
    # @api.depends('booking_type_id.event_videocall_source')
    # def _compute_videocall_source(self):
    #     events_no_booking = self.env['calendar.event']
    #     for event in self:
    #         if not event.booking_type_id or event.videocall_location and not self.DISCUSS_ROUTE in event.videocall_location:
    #             events_no_booking |= event
    #             continue
    #         event.videocall_source = event.booking_type_id.event_videocall_source
    #     super(CalendarEvent, events_no_booking)._compute_videocall_source()

    def _compute_is_highlighted(self):
        super(CalendarEvent, self)._compute_is_highlighted()
        if self.env.context.get('active_model') == 'booking.type':
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('booking_type_id'):
                if 'active' not in vals and vals.get('booking_status') == 'cancelled':
                    vals['active'] = False
                elif 'booking_status' not in vals and vals.get('active') is False:
                    vals['booking_status'] = 'cancelled'
        return super().create(vals_list)

    def write(self, vals):
        unconfirmed_bookings = self.filtered(lambda event: event.booking_type_id and event.booking_status != 'booked')
        if any(event.booking_type_id for event in self) or vals.get('booking_type_id'):
            if 'active' in vals and 'booking_status' not in vals:
                vals['booking_status'] = 'booked' if vals['active'] else 'cancelled'
            if 'active' not in vals and 'booking_status' in vals:
                vals['active'] = vals['booking_status'] != 'cancelled'

        res = super().write(vals)

        confirmed_bookings = unconfirmed_bookings.filtered(lambda event: event.booking_status == 'booked')
    ...
    def _inverse_resource_ids_or_capacity(self):
        ...
        for event in self:
            ...
            if resources:
                if event.booking_type_manage_capacity and self.resource_total_capacity_reserved:
                ...
                for resource in resources.sorted("shareable"):
                    if event.booking_type_manage_capacity and capacity_to_reserve <= 0:
                        break
    ...
    def _read_group_groupby(self, groupby_spec, query):
        """ Simulate group_by on resource_ids by using booking_resource_ids.
            booking_resource_ids is only used to store the data through the booking_booking_line
            table. All computation on the resources and the capacity reserved is done with capacity_reserved.
            Simulating the group_by on resource_ids also avoids to do weird override in JS on booking_resource_ids.
        ...
        if groupby_spec == 'resource_ids':
            return super()._read_group_groupby('booking_resource_ids', query)
        return super()._read_group_groupby(groupby_spec, query)

    def _read_group_booking_resource_ids(self, resources, domain):
        if not self.env.context.get('booking_booking_gantt_show_all_resources'):
            return resources
        resources_domain = [
            ('booking_type_ids', '!=', False), '|', ('company_id', '=', False), ('company_id', 'in', self.env.context.get('allowed_company_ids', [])),
        ]
        # If we have a default booking type, we only want to show those resources
        default_booking_type = self.env.context.get('default_booking_type_id')
        if default_booking_type:
            return self.env['booking.type'].browse(default_booking_type).resource_ids.filtered_domain(resources_domain)
        return self.env['booking.resource'].search(resources_domain)

    def _read_group_partner_ids(self, partners, domain):
        """Show the partners associated with relevant staff users in booking gantt context."""
        if not self.env.context.get('booking_booking_gantt_show_all_resources'):
            return partners
        booking_type_id = self.env.context.get('default_booking_type_id', False)
        booking_types = self.env['booking.type'].browse(booking_type_id)
        if booking_types:
            return booking_types.staff_user_ids.partner_id
        return self.env['booking.type'].search([('schedule_based_on', '=', 'users')]).staff_user_ids.partner_id

    def _read_group_user_id(self, users, domain):
        if not self.env.context.get('booking_booking_gantt_show_all_resources'):
            return users
        booking_types = self.env['booking.type'].browse(self.env.context.get('default_booking_type_id', []))
        if booking_types:
            return booking_types.staff_user_ids
        return self.env['booking.type'].search([('schedule_based_on', '=', 'users')]).staff_user_ids
    ...
    def _track_get_default_log_message(self, tracked_fields):
        if self.booking_type_id and 'active' in tracked_fields:
            if self.active:
                return _('Booking re-booked')
            else:
                return _("Booking cancelled")
    ...
    def action_cancel_meeting(self, partner_ids):
        ...
        """
        self.ensure_one()
        message_body = _("Booking cancelled")
        if partner_ids:
            ...
            if attendees:
                ...
                message_body = _("Booking cancelled by: %(partners)s", partners=cancelling_attendees)
    ...
    def _get_mail_tz(self):
        self.ensure_one()
        if not self.event_tz and self.booking_type_id.booking_tz:
            return self.booking_type_id.booking_tz
    ...
    def _track_template(self, changes):
        ...
        if not self.booking_type_id or self._skip_send_mail_status_update():
            return res

        booking_type_sudo = self.booking_type_id.sudo()
        ...
        if 'booking_type_id' in changes and self.booking_status == 'booked':
            try:
                booked_template = self.env.ref('booking.booking_booked_mail_template')
            except ValueError as e:
                _logger.warning("Mail could not be sent, as mail template is not found : %s", e)
            else:
                res['booking_type_id'] = (booked_template.sudo(), {
                    **vals,
                    'auto_delete_keep_log': False,
                    'subtype_id': self.env['ir.model.data']._xmlid_to_res_id('booking.mt_calendar_event_booked'),
                    'email_layout_xmlid': 'mail.mail_notification_light'
                })
        if (
            'active' in changes and not self.active and self.start > fields.Datetime.now()
            and booking_type_sudo.canceled_mail_template_id
        ):
            res['active'] = (booking_type_sudo.canceled_mail_template_id, {
                **vals,
                'auto_delete_keep_log': False,
                'subtype_id': self.env['ir.model.data']._xmlid_to_res_id('booking.mt_calendar_event_canceled'),
                'email_layout_xmlid': 'mail.mail_notification_light'
            })
        return res
    ...
    def _get_customer_description(self):
        ...
        confirmation_html = html_sanitize(self.booking_type_id.message_confirmation or '')
        base_url = self.booking_type_id.get_base_url()
    ...
    def _get_attendee_description(self):
        ...
        include_phone_partners = self.attendee_ids.filtered(lambda attendee: attendee.partner_id in (self.booking_booker_id + self.partner_id))
        ...
        questions = []
        answers = []
        # for question, answer in self.booking_answer_input_ids.sorted('id').grouped('question_id').items():
    ...
    def _get_customer_summary(self):
        ...
        if self.booking_type_id and self.booking_type_id.schedule_based_on == 'users' and self.partner_id:
            return _('%(booking_name)s with %(partner_name)s',
                     booking_name=self.booking_type_id.name,
    ...
    def _get_default_privacy_domain(self):
        ...
        Returns:
            The read_group privacy domain adapted to include every events related to a resource booking type.
        """
        domain = super()._get_default_privacy_domain()
        return expression.OR([domain, [
            '&',
            ('booking_type_id', '!=', False),
            ('booking_type_id.schedule_based_on', '=', 'resources')
        ]])

    @api.model
    def _gantt_unavailability(self, field, res_ids, start, stop, scale):
        # skip if not dealing with bookings
        if field not in ('resource_ids', 'partner_ids'):
            return super()._gantt_unavailability(field, res_ids, start, stop, scale)

        # if viewing a specific booking type generate unavailable intervals outside of the defined slots
        slots_unavailable_intervals = []
        booking_type = self.env['booking.type']
        if booking_type_id := self.env.context.get('default_booking_type_id'):
            booking_type = booking_type.browse(booking_type_id)
        ...
        if booking_type:
            ...
            slot_available_intervals = [
                ...
                for slot in booking_type._slots_generate(start_utc, stop_utc, 'utc', reference_date=start)
            ]
        ...
        booking_resource_ids = self.env['booking.resource'].browse(res_ids)
        # in multi-company, if people can't access some of the resources we don't really care
        if self.env.context.get('allowed_company_ids'):
            booking_resource_ids = booking_resource_ids.filtered_domain([
                '|', ('company_id', '=', False), ('company_id', 'in', self.env.context['allowed_company_ids'])]
            )

        resource_unavailabilities = booking_resource_ids.resource_id._get_unavailable_intervals(start, stop)

        result = {}
        for booking_resource_id in booking_resource_ids:
            unavailabilities = Intervals([
                (start, stop, set())
                for start, stop in resource_unavailabilities.get(booking_resource_id.resource_id.id, [])])
            unavailabilities |= Intervals([(start, stop, set()) for start, stop in slots_unavailable_intervals])
            result[booking_resource_id.id] = [{'start': start, 'stop': stop} for start, stop, _ in unavailabilities]
        return result
    ...
    def get_gantt_data(self, domain, groupby, read_specification, limit=None, offset=0, unavailability_fields=[], progress_bar_fields=None, start_date=None, stop_date=None, scale=None):
        ...
        if self.env.context.get('booking_booking_gantt_show_all_resources') and groupby and groupby[0] == 'partner_ids':
            staff_partner_ids = self.env['booking.type'].search([('schedule_based_on', '=', 'users')]).staff_user_ids.partner_id.ids
