...
class ir_attachment(models.Model):
    ...
    def _cmis_get(self):
        if self.type =='url':
            ...
            try:
                ...
                raise Warning("CMIS Error! couldn't load document.")
    ...
    def _cmis_set(self):
        ...
        repo = cmis_repo(self)
        if not self.env.user.get_cmis_client():
            # No repo configured. Fall back to normal.
            _logger.warn('\n\nFallback\n\n')
            return self._data_set('datas', self.datas, None)
        if not self.remote_id:
            try:
                ...
                self._data_set('datas', self.datas, None)
                return None
            try:
    ...
    def getFolder(self):
        self.ensure_one()
        res = self.parent_id.getFolder() if self.parent_id else self.env['document.directory'].model2dir(self).getFolder()
        _logger.warn('\n\nfolder: %s\n\n')
        return res
...
class document_directory(models.Model):
    ...
    def model2dir(self, attachment):
        ...
        models_directory = _check_dir(attachment.res_model or 'other', odoo_directory.id, self.env.ref('base.user_root').id,'/odoo_models/%s' % attachment.res_model)
...
class res_users(models.Model):
    ...
    def get_cmis_client(self):
        global CMIS_CLIENT, CMIS_LOGIN, CMIS_SERVER
        # TODO: This only works with one user. Fix it.
        return CMIS_CLIENT
