...
class CalendarBooking(models.Model):
    """ This model is only used to store booking informations for bookings WITH PAYMENT
    ...
    # Answers
    booking_answer_input_ids = fields.One2many('booking.answer.input', 'calendar_booking_id', string="booking Answers")
    # Calendar Event Data
    booking_invite_id = fields.Many2one('booking.invite')
    booking_type_id = fields.Many2one('booking.type', ondelete="cascade", required=True)
    ...
    # Payment
    account_move_id = fields.Many2one("account.move", string="booking Invoice", readonly=True)
    ...
    def unlink(self):
        ...
        answers = self.booking_answer_input_ids.filtered(lambda answer: not answer.calendar_event_id)
        answers.unlink()
        return super().unlink()
    ...
    def _filter_unavailable_bookings(self):
        ...
        for index, start in enumerate(boundaries[:-1]):
            ...
            for resource, booking_lines in interval_bookings.booking_line_ids.grouped("booking_resource_id").items():
                booking_lines &= available_resource_bookings.booking_line_ids
                total_resource_availability = resource.booking_type_ids[0]._get_resources_remaining_capacity(
                    resource, start, stop, with_linked_resources=False
                )['total_remaining_capacity'] if booking_lines and resource.booking_type_ids else 0
    ...
    def _get_description(self):
        ...
        self.ensure_one()
        tz = self.env.context.get('tz') or self.partner_id.tz or self.booking_type_id.booking_tz
        env_tz = self.with_context(tz=tz).env
        if self.staff_user_id:
            return _(
                "%(name)s with %(staff_user)s\n%(date_start)s at %(time_start)s to\n%(date_end)s at %(time_end)s (%(timezone)s)",
                name=self.booking_type_id.name,
    ...
    def _make_event_from_paid_booking(self):
        ...
        for booking in todo - unavailable_bookings:
            booking_line_values = [{
                'booking_resource_id': line.booking_resource_id.id,
                'capacity_reserved': line.capacity_reserved,
                'capacity_used': line.capacity_used,
            } for line in booking.booking_line_ids]

            calendar_event_values = booking.booking_type_id._prepare_calendar_event_values(
                booking.asked_capacity, booking_line_values, booking.duration, booking.booking_invite_id,
                booking.guest_ids, booking.name, booking.partner_id, booking.staff_user_id, booking.start, booking.stop
            )
            calendar_event_values['booking_answer_input_ids'] = [Command.set(booking.booking_answer_input_ids.ids)]
