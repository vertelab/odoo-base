...
class BookingType(models.Model):
    ...
    def _get_booking_slots(self, timezone, filter_users=None, filter_resources=None, asked_capacity=1, reference_date=None):
        # get available slots from parent
        ...
        # Add boat details to available resources
        for month in slots:
    ...
    def _slot_availability_is_resource_available(self, slot, resource, availability_values):
        # get availability from parent
        available = super()._slot_availability_is_resource_available(slot, resource, availability_values)

        if not available or self.booking_type != 'boat':
            return available
        ...
        if L is not None and resource.length < L:
            return False
        if W is not None and resource.width < W:
            return False
        if D is not None and resource.depth < D:
            return False
